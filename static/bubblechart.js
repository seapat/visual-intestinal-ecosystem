...
function groupBy (columnName) {
    return d3.groups(dataset, key => (key[columnName] == null) ? "Unknown" : key[columnName])
}
...
function sortByMean (groupedData, columnName=null) {
    if (columnName != null) { 
        return d3.sort(groupedData, d => d3.mean(d[1], value => value[columnName]));
    }
    else {
...
// console.log(sortByMean(countOccurences("Nationality"), "Age"));
...
console.log(getMax("Nationality"));

// VARIABLES //

//extract nationality column, used for x axis and scaling bubble size,
let nationalities = extractColumn("Nationality"); //used for multiple operations
let nationCounts = countOccurences(nationalities);
...
// X AXIS //
    ...
    let xScale = d3.scalePoint()
        .domain(
            Array.from( //create Array of Nation Names
                sortByMean(groupBy("Nationality"), "Age"), //group by nation, sort nations by average age
                nation => nation[0] //extract nation name
                ) 
            )
        .range([0, width])
        .padding([0.5]);
...
// DRAW BUBBLES //
    ...
    /* 
    console.log(Array.from( //create Array of Nation Names
        sortByMean(groupBy("Nationality"), "Age"), //group by nation, sort nations by average age
        nation => nation[1].length)) 
        */

    // scale bubble size
    let bubbleScale = d3.scaleSqrt()
        .domain([1, 10])
        .range([1 , 10]); //getMax(nationCounts)


        console.log(sortByMean(groupBy("Nationality"), "Age"))
        svg.append('g')
        .selectAll("dot")
        .data(sortByMean(groupBy("Nationality"), "Age"))
        .enter()
        .append("circle")
        .attr("class",  function(d) { 
            return "bubbles " + d[0];}) //returns names of nations
        .attr("cx", function (d) { 
            return xScale(d[0]);}) //position according to names of nations
        .attr("cy", function (d) { 
            return yScale(d3.mean(d[1], d => d.Age)); } ) //loop through object/subjects per nation, calculate mean of Age
        .attr("r", function (d) { 
